...
def PreProcessTask(igortask, full_blast_info=False, keep_stop_codon=False, igdata=None, verbose=True):
    ...
    Align_Seqs(specie=specie, receptor=receptor,
               pr_pr_batchname=pr_pr_batchname, igdata=igdata)
    if verbose is True:
...
def Align_Seqs(specie, receptor, pr_pr_batchname, igdata=None):
    ...
    args = ["pyir", filein, "-o", fileout, "--outfmt",
            "tsv", "-r", receptor, "-s", specie]
    if igdata != None:
        if self.verbose:
...
def Process_Seqs(pr_pr_batchname, full_blast_info=False, keep_stop_codon=False):
    ...
    aligned = pd.read_csv(filein, sep="\t", compression="gzip",
                          dtype=str, index_col=['sequence_id'])
    ...
    processed.to_csv(f'{pr_pr_batchname}.csv.gz', columns=[
                     "sequence"], compression="gzip")
