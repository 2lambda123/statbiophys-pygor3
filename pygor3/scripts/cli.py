...
def common_options(f):
    options = [
        click.option("-s", "--set_igor_species", "igor_species",
                     default=None, help="Species in IGoR's format: human, mouse"),
        click.option("-c", "--set_igor_chain", "igor_chain", default=None,
        ...
        click.option("--Janchors", "fln_J_gene_CDR3_anchors", default=None,
                     help="V gene anchors filename"),
        click.option("-w", "--set_wd", "igor_wd",
                     help="Sets the working directory to path", default='./', show_default=True),
        click.option("-b", "--set_batch", "igor_batch", default=None,
                     help='Sets batchname to identify run. If not set random name is generated'),
        click.option("-D", "--set_database", "igor_fln_db", default=None,
...
def run_read_seqs(igor_read_seqs, igor_species, igor_chain, igor_model, igor_model_path, igor_path_ref_genome, igor_wd, igor_batch, igor_fln_db,
    ...
    Q_model_files = (not (igor_model_parms is None)
                     and not (igor_model_marginals is None))
    if Q_species_chain:
        igortask.igor_species = igor_species
        igortask.igor_chain = igor_chain
    elif Q_model_files:
...
def run_generate(N,
    ...
    if fln_output_prefix is None:
        ...
        igortask.igor_fln_generated_realizations_werr = None
        igortask.igor_fln_generated_seqs_werr = igortask.igor_wd + "/" + \
            igortask.igor_batchname + "_generated/generated_seqs_werr.csv"
        igortask.igor_fln_generated_realizations_werr = igortask.igor_wd + "/" + \
            igortask.igor_batchname + "_generated/generated_realizations_werr.csv"
        ...
        try:
            ...
            if (rec_type == 'VDJ'):
                if igortask.igor_path_ref_genome is None:
                    ...
                    p3imgt.download_ref_genome_VDJ(
                        imgt_species, imgt_chain, modelspath=None)
            elif (rec_type == 'VJ'):
                if igortask.igor_path_ref_genome is None:
                    p3imgt.download_ref_genome_VJ(imgt_species, imgt_chain)
                else:
                    p3imgt.download_ref_genome_VJ(
                        imgt_species, imgt_chain, modelspath=None)
            else:
    ...
    if b_to_txt:
        try:
            if fln_to_txt[1] is not None:
                ...
                mdl_to.marginals.write_model_marginals(fln_to_txt[1])
                print(
                    "WARNING: No marginals file was specified, a uniform distribution was set and save in file ", fln_to_txt[1])
            print(mdl_to)
        except Exception as e:
...
def database_import(fln_output_db,
    ...
    if (igortask.igor_species is not None) and (igortask.igor_chain is not None):
        print("species : ", igortask.igor_species,
              " chain: ", igortask.igor_chain)
        try:
        ...
        try:
            ...
            for table_pattern in sql_tablename_patterns_dict['ref_genome']:
                ...
                for tablename_to_copy in tablename_list:
                    ...
                    # Copy table
                    fln_source_db = igortask.igor_db.flnIgorDB
                    output_db.copytable_from_source(
                        tablename_to_copy, fln_source_db)
        except Exception as e:
...
def database_export(b_airr_rearrangement, fln_airr_rearrangement,
    ...
    if ii_db.Q_model_in_db():
        print("MODEL : ", igortask.igor_model_parms_file,
              igortask.igor_model_marginals_file)
        ii_db.write_IgorModel_to_TXT(
            igortask.igor_model_parms_file, igortask.igor_model_marginals_file)
    # b_igor_model_parms
...
def database_naive_align(output_fln, seq_index, igor_fln_db):
    ...
    if seq_index is None:
        ...
        def generate_str_fasta(indexed_sequence, list_vdj_alignments: dict):
            ...
            str_fasta = ""
            min_offset_key = min(list_vdj_alignments.keys(
            ...
            str_fasta = str_fasta + "> " + \
                str(indexed_sequence.seq_index) + "\n"
            ...
            for key in list_vdj_alignments.keys():
                ...
                str_fasta_sequence = str_prefix + \
                    list_vdj_alignments[key].strGene_seq
                print(str_fasta_sequence)
                ...
                print("offset_5_p : ",
                      list_vdj_alignments[key].offset_5_p, offset_5_p)
                print("offset_3_p : ",
                      list_vdj_alignments[key].offset_3_p, offset_3_p)
        ...
        try:
            ...
            print("ERROR: seq_index ", seq_index,
                  " not found in db ", igortask.igor_fln_db)
            raise
        indexed_sequence.offset = 0
        ...
        if output_fln is None:
            ...
            fln_output = batchname + '__' + \
                str(indexed_sequence.seq_index) + '_na.fasta'
...
def database_plot_pgen():
    ...
    for flnIGoRCDR3 in args:
        ...
        hist, bin_edges = np.histogram(dataCDR3LenArr, bins=range(
            dataCDR3LenArr.min(), dataCDR3LenArr.max()))
        ...
        type(bin_edges[1:])
        xbins
        np.savetxt(flnIGoRCDR3_hist, zip(
            bin_edges[:-1], bin_edges[1:], hist), fmt='%d')
